...
func (t *Service1) Multiply(r *http.Request, req *ServiceRequest, res *ServiceResponse) error {
...
func (t *Service3) Add(r *http.Request, req *ServiceRequest, res *ServiceResponse) error {
	res.Result = req.A + req.B
	return nil
}
...
func (c MockCodec) NewRequest(*http.Request) Request {
	return MockCodecRequest{c.A, c.B, c.Method}
}
...
func TestServeHTTP(t *testing.T) {
	...
	s.RegisterCodec(MockCodec{A, B, "Service1/multiply"}, "mock")
...
func TestValidationFails(t *testing.T) {
	...
	s := NewServer()
	s.RegisterService(new(Service1), "")
	s.RegisterCodec(MockCodec{1, 2, "Service1/multiply"}, "mock")
	s.RegisterValidateRequestFunc(validate)
...
func TestRegisterMethodSuccessful(t *testing.T) {
	const (
		A = 2
		B = 3
	...
	s := NewServer()
	s.RegisterMethod(new(Service3), "v1/calc/sum2", "Add")
	s.RegisterCodec(MockCodec{A, B, "v1/calc/sum2"}, "mock")
	s.RegisterValidateRequestFunc(validate)
...
func TestAliasSuccessful(t *testing.T) {
	const (
		A = 2
		B = 3
	...
	s := NewServer()
	s.RegisterMethod(new(Service3), "v1/calc/sum2", "Add")
	s.RegisterAlias("v2/calc/sum", "v1/calc/sum2")
	s.RegisterCodec(MockCodec{A, B, "v2/calc/sum"}, "mock")
	s.RegisterValidateRequestFunc(validate)
